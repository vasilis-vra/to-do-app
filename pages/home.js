...
export default function Home() {
  ...
  const addTask = async (tasksId, value, list) => {
    ...
    if (res.result.nModified === 1 && list === "today_tasks") {
  ...
  const removeTask = async (tasksId, task) => {
    const data = await fetch(
      `/api/deleteTask?_id=${tasksId}&list=today_tasks&value=${task}`
    );
    const res = await data.json();
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await fetch(
        `/api/findTasks?email=${session.user.email}&list=today_tasks`
      );
      const json = await res.json();
      if (json) {
        setContent(json.today_tasks);
        setTasksId(json._id);
      }
    };
    fetchData();
    ...
    return (
      <div className={styles.container}>
        <span className="page-header">
          <h2 className="d-inline">Today - </h2>
          <h6 className="d-inline">{today}</h6>
        </span>
        <hr />
        <main>
                ...
                >
                  ...
                  <Dropdown>
                    ...
                    <Dropdown.Menu>
                      <Dropdown.Item
                        onClick={() => {
                          addTask(tasksId, task, "inbox_tasks");
                          removeTask(tasksId, task);
                        }}
                      >
                        Move to Inbox
