...
export default function Home() {
  ...
  const addTask = async (tasksId, value, list) => {
    ...
    if (res.result.nModified === 1 && list === "future_tasks") {
  ...
  const removeTask = async (tasksId, task) => {
    const data = await fetch(
      `/api/deleteTask?_id=${tasksId}&list=future_tasks&value=${task}`
    );
    const res = await data.json();
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await fetch(
        `/api/findTasks?email=${session.user.email}&list=future_tasks`
      );
      const json = await res.json();
      if (json) {
        setContent(json.future_tasks);
        setTasksId(json._id);
      }
    };
    fetchData();
    ...
    return (
      <div className={styles.container}>
        <span className="page-header">
          <h2 className="d-inline">Future</h2>
        </span>
        <hr />
        <main>
